...
export function BlogCard({ blog }: { blog: BlogMetadata }) {
  ...
  return (
    <Link href={`/blogs/${blog.slug}`} className="block">
      <article className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-emerald-300/60 cursor-pointer">
